# Validation Playlist: Core Connectivity

## Purpose
Quick, repeatable checks for DNS, reachability, HTTPS, and TLS trust across core
blueprint services.

## Preconditions
- Internal CA installed on the controller workstation.
- Hostnames resolve via edge firewall or directory services.
...
- All core services show DNS + PING `OK`.
- TLS status is `OK` for CA-backed services.
- Log file saved under `docs/logs/`.

## Rollback/Revert
Not applicable (read-only checks). Fix the underlying service or revert the
latest change and re-run the playlist.
